...
module.exports = {
  ...
  auth: {
    ...
    qiniu_add_time_prefix: {
      label: 'time as prefix',
      type: 'enum',
      values: ['No', 'Yes'],
    },
    qiniu_debug: {
  ...
  init: conf => {
    ...
    let prefix = conf.qiniu_key_prefix
  
    return {
      upload: file => new Promise((resolve, reject) => {
        let key = `${prefix}/${encodeURIComponent(file.sha256)}`
        if (conf.qiniu_add_time_prefix === 'Yes') {
          key = `${key}-${Date.now()}`
        }
      ...
      delete: file => {
        let key = file.url.replace(`${conf.qiniu_base_url}/`, '')
